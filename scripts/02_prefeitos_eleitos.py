"""
Baixa dados do TSE e extrai prefeitos eleitos em 2020 e 2024.
Fonte: dados.tse.jus.br — consulta_cand
Saída: prefeitos_eleitos_2020_2024.csv
"""
...
def baixar_e_filtrar(ano: int) -> pd.DataFrame:
    ...
    # Prefeito = CD_CARGO 11
    df = df[df["CD_CARGO"].str.strip() == "11"]
    print(f"[{ano}] {len(df):,} candidatos a prefeito")

    # Apenas eleitos
    df = df[df["DS_SIT_TOT_TURNO"].str.upper().str.startswith("ELEITO", na=False)]
    print(f"[{ano}] {len(df):,} prefeitos ELEITOS")
...
saida = "prefeitos_eleitos_2020_2024.csv"
